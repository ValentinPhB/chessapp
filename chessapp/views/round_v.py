...
class RoundMakerView:
    ...
    def welcome(current_tournament):
        ...
        print(f"\n\n>>> CRÉATION DU ROUND '{nb_round}' sur"
              f" '{nb_total_round}' ET DE SES MATCHES. <<<\n")
    ...
    def precedent_matches_message(current_tournament):
        ...
        print(f"VOICI LES MATCHS POUR LE ROUND '{nb_actual_round}'"
              f" sur '{nb_total_round}' "
              f"QUE VOUS AVEZ GÉNÉRÉ :")
    ...
    def display_matches(current_tournament):
        ...
        for element in list_matches:
            print(f"\n> MATCH {i} :"
                  f" '{element.first_p.family_name}"
                  f" {element.first_p.first_name}'"
                  f" né le : '{element.first_p.date_of_birth}'"
                  f" classement du tournoi"
                  f" '{(all_players.index(element.first_p)) + 1}'"
                  f" | CONTRE |"
                  f" '{element.second_p.family_name}"
                  f" {element.second_p.first_name}'"
                  f" né le : '{element.second_p.date_of_birth}'"
                  f" classement du tournoi"
                  f" '{(all_players.index(element.second_p)) + 1}' ")
            i += 1
