...
class RoundMakerView:
    ...
    def precedent_matches_message(current_tournament):
        ...
        print(f"VOICI LES MATCHS POUR LE ROUND '{nb_actual_round}' sur '{nb_total_round}' "
    ...
    def display_matches(current_tournament):
        ...
        for element in list_matches:
            print(f"\n> MATCH {i} :"
                  ...
                  f" né le : '{element.first_p.date_of_birth}' classement du tournoi"
                  f" '{(all_players.index(element.first_p)) + 1}'"
                  f" | CONTRE |"
                  f" '{element.second_p.family_name} {element.second_p.first_name}'"
                  f" né le : '{element.second_p.date_of_birth}' classement du tournoi"
