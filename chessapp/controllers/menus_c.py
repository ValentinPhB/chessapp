...
from chessapp.controllers.rapports_c import RapportController
...
class HomeMenuController:
    ...
    def __call__(self):
        ...
        if self.db.tournaments_table.contains(where('date_end') == str("None")):
            ...
            self.menu.add("auto", "Afficher des rapports.", RapportController())
...
class AddPlayerMenuController:
    ...
    def __call__(self):
        ...
        # ADD MENUS.
        self.menu.add("auto", "Ajouter un nouveau joueur et le sauvegarder dans la base de données.",
...
class PlayerDataMenuController:
    ...
    def __call__(self):
        ...
        # SELECTING PLAYERS FROM DATA AND SHOW ONLY THOSE WHO ARE NOT ALREADY CHOSEN.
        # ADDING EVERY PLAYER AS MENUS.
        number_player = self.db.players_table.all()
        if len(number_player) >= 1:
            player_to_show = sorted(number_player, key=lambda k: int(k['ranking']))
            if len(self.tournament.players_tournament) > 0:
                for player_instance in self.tournament.players_tournament:
                    ...
                    if self.db.players_table.contains((where('family_name') == family_name)
                        ...
                        player_to_show[:] = [x for x in player_to_show if not ((x.get('family_name') == family_name)
            ...
            # OTHERWISE IT WILL INFORM THE USER AND REDIRECT IT TO 'ADD PLAYER MENU CONTROLLER'.
...
class ContinueMenuController:
    ...
    def __call__(self):
        ...
        # OTHERWISE IT WILL INFORM THE USER AND REDIRECT IT TO 'HOME MENU CONTROLLER'.
        if len(self.db.tournaments_table) >= 1:
            for dict_tournament in self.db.tournaments_table:
                if dict_tournament['date_end'] == "None":
                    self.menu.add("auto", f"NOM : '{dict_tournament['name']}',"
                                          f" LIEU : '{dict_tournament['place']}',"
                                          f" DATE DE DEBUT : '{dict_tournament['date_start']}',"
                                          f" NOMBRE TOTAL DE ROUNDS : '{dict_tournament['nb_total_round']}',"
                                          f" ROUND(S) JOUÉ(S) : '{len(dict_tournament['all_round'])}'.",
                                  ContinueTournamentController(dict_tournament))
                else:
                    pass
...
class SuggestRankingMenuController:
    """
    This controller ask if the user wants to update player's ranking, continue the tournament or save and quit.
    """
    ...
    def __call__(self):
        ...
        elif int(self.tournament.nb_total_round) == len(self.tournament.all_round)\
                and (precedent_round.matches[0].result_1 != precedent_round.matches[0].result_2
                     or precedent_round.matches[0].result_1 == precedent_round.matches[0].result_2 == 0.5):
